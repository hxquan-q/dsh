...
describe('threshold escalation', () => {
  ...
  it('vetoes at the default stopAfter (8) after reminding at 3 and 5', async () => {
    ...
    const results = toolResults(agent)
    expect(results[7]!.data.message.content[0]!.content).toEqual(
      expect.arrayContaining([expect.objectContaining({ type: 'text', text: expect.stringContaining('Repeated tool call blocked:') })]),
    )
  })
